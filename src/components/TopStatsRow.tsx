  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-5 mb-5">
      {topStats.map((stat: any) => {
        ...
        return (
          <Card key={stat.id} className="p-4.5 sm:p-5 flex flex-col justify-between group hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
            <div className="flex items-center justify-between mb-3 gap-1.5">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className={`p-2 rounded-xl transition-colors duration-200 shrink-0 ${config.bg}`}>
                  {config.icon}
                </div>
                <span className="text-sm font-bold text-slate-700 tracking-wide truncate">{stat.title}</span>
              </div>
              {stat.arch && (
                <span className="text-xs font-semibold text-slate-600 bg-slate-100/90 px-2 py-0.5 rounded-md border border-slate-200/80 shrink-0">
                  {stat.arch}
                </span>
              )}
            </div>
